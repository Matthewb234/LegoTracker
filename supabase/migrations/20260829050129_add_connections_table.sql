...
ALTER TABLE "public"."connections"
    ADD CONSTRAINT "connections_no_self_check" CHECK ("requester_id" <> "addressee_id");
...
-- one row per pair, regardless of who requested
CREATE UNIQUE INDEX "connections_unique_pair_idx" ON "public"."connections" (
                                                                             LEAST("requester_id", "addressee_id"),
                                                                             GREATEST("requester_id", "addressee_id")
    );

-- both directions get queried, so index both columns
